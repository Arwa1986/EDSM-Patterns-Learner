...
from Pattrens.negative_patterns2 import *
...
class Learner:
        ...
        for blue in self.blue_states:
            for red in self.red_states:
                ...
                if is_valid_merge(self.apta, ds):
                    ...
                    merging_scour = self.compute_score_EDSM(ds)
    ...
    def run_EDSM_usrDefNegPat_learner(self, usrDefNegativePatterns):
        if self.apta.is_all_states_red():
            return
        ...
        for blue in self.blue_states:
            for red in self.red_states:
                ...
                if is_valid_merge(self.apta, ds):
                    # merging_scour = self.compute_scour_with_negative_patterns(ds)
                    merging_scour = self.compute_score_usrDefNegPat(usrDefNegativePatterns,ds)
                    ds.merging_scour = merging_scour
                    mergable_states.append(ds)
                    if merging_scour > 0 :
                        # ds.printSets()
    ...
    def pick_next_blue2(self, red):
            ...
            if red in self.red_states:
                ...
                # Exclude red states
                self.blue_stall_properties, avgScour, reference_DFAates = [s for s in all_states if self.apta.G.nodes[s].get('fillcolor') != 'red']
    ...
    def compute_score_EDSM(self, ds):
        merging_score = 0
        all_sets = ds.get_sets()
        for representative, elements in all_sets.items():
            if len(elements)>1:
                merging_score += (len(elements)-1)

        return merging_score -1

    def compute_score_usrDefNegPat(self, negative_patterns, ds):
        merging_score = 0
        all_sets = ds.get_sets()
        for representative, elements in all_sets.items():
            if len(elements) > 1:
        ...
        if merging_score > -1:
            ...
            if has_negative_patterns(self.apta, negative_patterns, statesOfInterest):
